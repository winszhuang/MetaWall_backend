...
async function signUp(req, res, next) {
  ...
  if (!email || !password || !name) return appError(404, '請完整輸入欄位', next);

  if (!validator.isLength(name, { min: 2 })) return appError(404, '暱稱需大於2字元', next);

  if (!validator.isEmail(email)) return appError(404, 'email格式錯誤', next);
  ...
  if (isUserExist) return appError(404, '帳號已被註冊，請替換新的Email !', next);
...
async function signIn(req, res, next) {
  ...
  if (!password || !email) return appError(404, '請完整輸入欄位', next);

  const currentUser = await User.findOne({ email }).select('+password');
  if (!currentUser) return appError(404, 'email不存在，請註冊再登入 !', next);
